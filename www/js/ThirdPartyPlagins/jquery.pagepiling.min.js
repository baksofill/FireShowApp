...
(function (b) {
    b.fn.pagepiling = function (c) {
        ...
        function g(a, f) {
            ...
            d.activeSection.is(a) || ("undefined" === typeof d.animated && (d.animated = !0), "undefined" !== typeof d.anchorLink && C(d.anchorLink,
                d.sectionIndex), d.destination.addClass("active").siblings().removeClass("active"), d.sectionsToMove = D(d), "down" === d.yMovement ? (d.translate3d = "vertical" !== c.direction ? "translate3d(-100%, 0px, 0px)" : "translate3d(0px, -100%, 0px)", d.scrolling = "-100%", c.css3 || d.sectionsToMove.each(function (a) {
                a != d.activeSection.index(".pp-section") && b(this).css(m(d.scrolling))
            }), d.animateSection = d.activeSection) : (d.translate3d = "translate3d(0px, 0px, 0px)", d.scrolling = "0", d.animateSection = a), b.isFunction(c.onLeave) && c.onLeave.call(this,
                d.leavingSection, d.sectionIndex + 1, d.yMovement), E(d), F(d.anchorLink), G(d.anchorLink, d.sectionIndex), r = d.anchorLink, s = (new Date).getTime())
        }
        ...
        function C(a,
                   b) {
            c.anchors.length ? (location.hash = a, v(location.hash)) : v(String(b))
        }
        ...
        function h(a) {
            if (!p()) {
                ...
                var c = b(".pp-section.active"), c = w(c);
                0 > a ? k("down", c) : k("up", c);
                return !1
            }
        }
        ...
        function I(a) {
            ...
            z(a.target) || (a.preventDefault(), a = b(".pp-section.active"), a = w(a), p() || (f = y(f), touchEndY = f.y, touchEndX = f.x, "horizontal" === c.direction && Math.abs(touchStartX - touchEndX) > Math.abs(l - touchEndY) ? Math.abs(touchStartX - touchEndX) > e.width() / 100 * c.touchSensitivity &&
            (touchStartX > touchEndX ? k("down", a) : touchEndX > touchStartX && k("up", a)) : Math.abs(l - touchEndY) > e.height() / 100 * c.touchSensitivity && (l > touchEndY ? k("down", a) : touchEndY > l && k("up", a))))
        }

        function z(a, f) {
            f = f || 0;
            var d = b(a).parent();
            return f < c.normalScrollElementTouchThreshold && d.is(c.normalScrollElements) ? !0 : f == c.normalScrollElementTouchThreshold ? !1 : z(d, ++f)
        }
        ...
        var e = b(this), r, s = 0, A = "ontouchstart"in window || 0 < navigator.msMaxTouchPoints || navigator.maxTouchPoints, l = touchStartX = touchEndY = touchEndX = 0;
        c = b.extend({
            direction: "vertical",
            menu: null,
            verticalCentered: !0,
        ...
        b.fn.pagepiling.setMouseWheelScrolling = function (a) {
            a ? e.get(0).addEventListener ? (e.get(0).addEventListener("mousewheel", h, !1), e.get(0).addEventListener("wheel", h, !1)) : e.get(0).attachEvent("onmousewheel", h) : e.get(0).addEventListener ?
                (e.get(0).removeEventListener("mousewheel", h, !1), e.get(0).removeEventListener("wheel", h, !1)) : e.get(0).detachEvent("onmousewheel", h)
        };
        b.fn.pagepiling.setAllowScrolling = function (a) {
            a ? (b.fn.pagepiling.setMouseWheelScrolling(!0), A && (MSPointer = x(), e.off("touchstart " + MSPointer.down).on("touchstart " + MSPointer.down, H), e.off("touchmove " + MSPointer.move).on("touchmove " + MSPointer.move, I))) : (b.fn.pagepiling.setMouseWheelScrolling(!1), A && (MSPointer = x(), e.off("touchstart " + MSPointer.down), e.off("touchmove " +
            MSPointer.move)))
        };
        ...
        b.fn.pagepiling.moveTo = function (a) {
            var c = "", c = isNaN(a) ? b('[data-anchor="' + a + '"]') : b(".pp-section").eq(a -
            1);
            0 < c.length && g(c)
        };
        ...
        b.isEmptyObject(c.navigation) || J();
